...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    float nms=.45;
    //更改以测试更多的图片
    char **test_files_list=get_labels("data/test.txt");
    int index=0;
    while(test_files_list[index]!=NULL){
//    while(1){
        printf("This is the %d images\n...",index+1);
        filename=test_files_list[index];
        char new_name[20]={'\0'};
        if(filename){
//            int i=0;
//            for (i=16;filename[i];i++)
//                new_name[i-16]=filename[i];
            strncpy(new_name,&filename[strlen(filename)-9],5);
